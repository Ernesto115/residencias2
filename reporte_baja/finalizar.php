...
$eval_cuidado_vehiculo =
    (int)($_POST['eval_cuidado_vehiculo'] ?? 0);

$eval_productividad =
    (int)($_POST['eval_productividad'] ?? 0);

$eval_rendimiento =
    (int)($_POST['eval_rendimiento'] ?? 0);

$eval_cuidado_fisico =
    (int)($_POST['eval_cuidado_fisico'] ?? 0);
...
$stmt = $db->conn->prepare(
    "SELECT
        ...
        o.estatus estatus_operador,
        o.id_empresa id_empresa_operador
...
$id_empresa_operador = (int)$reporte['id_empresa_operador'];
$id_operador = (int)$reporte['id_operador'];


/* OPERADOR NO DEBE HABER CAMBIADO DE EMPRESA */
if ($id_empresa_operador !== $id_empresa_reporte) {
    responder(
        false,
...
$promedio_servicio = round(
    ($eval_distancia+$eval_tiempo+$eval_ganancias)/3,
    2
);
