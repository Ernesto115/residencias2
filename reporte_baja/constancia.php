...
/* PERMISOS */
if (!in_array($rol, ['ADMIN', 'ADMINISTRADOR', 'PROPIETARIO'])) {
    die('No tienes permiso para consultar esta constancia.');
}

if ($id_reporte <= 0) die('Reporte no válido.');

/* OBTENER INFORMACIÓN */
$sql = "SELECT rb.id_empresa, rb.fecha_ingreso, rb.fecha_baja, rb.estatus_evaluacion,
        o.nombres, o.primer_apellido, o.segundo_apellido, o.rfc,
        e.nombre_empresa, e.razon_social, e.direccion_fiscal, e.responsable
        ...
        WHERE rb.id_reporte = :id LIMIT 1";

$stmt = $db->conn->prepare($sql);
$stmt->execute([':id' => $id_reporte]);
$datos = $stmt->fetch(PDO::FETCH_ASSOC);

if (!$datos) die('No se encontró el reporte.');
if ($datos['estatus_evaluacion'] !== 'COMPLETADA') die('La baja todavía no ha sido completada.');

/* VALIDAR EMPRESA DEL PROPIETARIO */
if ($rol === 'PROPIETARIO') {
    ...
    if ($multiempresa === 1) {
        $stmt = $db->conn->prepare(
            "SELECT 1 FROM usuario_empresas
             WHERE id_usuario = :usuario AND id_empresa = :empresa LIMIT 1"
        );
        $stmt->execute([':usuario' => $id_usuario, ':empresa' => $id_empresa]);

        if (!$stmt->fetchColumn()) die('No tienes permiso para consultar esta constancia.');

    } elseif ($id_empresa_sesion !== $id_empresa) {
        die('No tienes permiso para consultar esta constancia.');
    }
}
...
function fechaBonita($fecha) {
    return $fecha ? date('d/m/Y', strtotime($fecha)) : 'No disponible';
}
...
<body class='pdf-body'>
    ...
    <div class='pdf-marco'>
        ...
        <h1 class='pdf-titulo'>CONSTANCIA LABORAL</h1>
        ...
        <div class='pdf-fecha'>
            Fecha de expedición: <strong>$fechaHoy</strong>
        </div>
...
/* FALSE = ABRIR EN EL VISOR, NO DESCARGAR AUTOMÁTICAMENTE */
$pdf->stream(
